...
namespace my{
    void find_duplicates(std::string *name, std::string *datetime){
        ...
        // getting map of hashes:
        std::string file_and_path = fs::current_path().string() + "/" + *name + "/.cppSync/hashes/" + *datetime;
        auto stringstream = std::ostringstream{};
        std::ifstream input_file(file_and_path);
        ...
        std::string newpath = oldpath + "/" + *name + "/.cppSync/duplicates";
        fs::current_path(newpath);
        std::ofstream duplicates(*datetime);
